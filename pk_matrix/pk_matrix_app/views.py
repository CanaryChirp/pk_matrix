...
def ai_settings(request):
    ...
    form_class = AISettingsForm(request.POST or None, profile_set=prof_set)
    #selected = form_class.fields['Profile to edit:']
    #selected = dict(form_class.fields['Profile to edit:'].choices)[selected]


    if form_class.is_valid():
        selected = form_class.cleaned_data['Profile to edit:']
    else:
        selected = 99
    ...
    if request.method == "POST":
        ...
        elif "delete_prof" in request.POST:
            if prof_set.ai_profile_set.count() > 1:
                prof_set.ai_profile_set.get(pk=selected).delete()
        elif "sub_edits" in request.POST:
            profile = prof_set.ai_profile_set.get(pk=selected)
            percentage = form_class.cleaned_data['percentage']
            profile.percentage = percentage
            profile.save()

    form_class.refresh(profile_set=prof_set)
    if selected != 99:
        update_form(form_class, prof_set.ai_profile_set.get(pk=selected))

    return render(request, 'ai_settings.html', {'form': form_class, 'selected': selected})

def update_form(form, profile):
    form.fields['percentage'].initial = profile.percentage
